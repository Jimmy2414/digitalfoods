import {GET_RECIPES, GET_TYPES, POST_RECIPES, GET_STATE_ID, GET_RECIPES_NAME,  
    FILTER_BY_SEARCHBAR, FILTER_BY_DIETS,  FILTER_BY_ORDER, ORDER_BY_SCORE, GET_RECIPES_ID
             ...
             case GET_RECIPES_NAME:
                            const addRecipe = state.recipesAll
                              return {
                                        ...
                                        recipes:Array.isArray(action.payload) ? action.payload : [action.payload],
...
const recipesfilter= action.payload === 'SSc' ? state.recipes.sort((a, b) => {
    ...
    if ((a.score - b.score) < 0) return 1
                    ...
                    console.log(recipesfilter.length)
                                        return{
                                            ...state,
                                            recipes: recypesByScore.length === recipesfilter.length? recypesByScore : recipesfilter 
                        ...
                        const recypesByOrder = action.payload === 'up' ? state.recipesAll.sort((a, b) => {
                                 if (a.name.toLowerCase() > b.name.toLowerCase()) return 1
                                           ...
                                           if (a.name.toLowerCase() < b.name.toLowerCase()) return 1
                                            ...
                                            const recypesByOrfil = action.payload === 'up' ? state.recipes.sort((a, b) => {
                                                if (a.name.toLowerCase() > b.name.toLowerCase()) return 1
                                                else return -1
                                                   }): state.recipes.sort((a, b) => {
                                                          if (a.name.toLowerCase() < b.name.toLowerCase()) return 1
                         ...
                         return{
                           ...state,
                          recipes:  recypesByOrder.length === recypesByOrfil.length? recypesByOrder : recypesByOrfil 
                        }   
            ...
            case FILTER_BY_DIETS:
              ...
              console.log(recipe.diets)
                    ...
                    const filter2 = action.payload === 'Filter by type' ? 
                    state.recipesAll
