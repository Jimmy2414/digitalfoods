...
const infoapi = async () => {
    try
    {
        const url = await axios.get(`${URLFOOD}/recipes/complexSearch?apiKey=${YOUR_APY_KEY}&addRecipeInformation=true&number=100`);

        // 'https://api.spoonacular.com/recipes/complexSearch?apiKey=a675acac718c4bfc989cb8399efe9578&addRecipeInformation=true&number=10'  
        const info =  await url.data.results.map(c => {
                return {
                    ...
                    summary:c.summary, 
                    steps: (c.analyzedInstructions[0] && c.analyzedInstructions[0].steps?c.analyzedInstructions[0].steps.map(item=>item.step).join("*Siguiente Paso:"):'')
                }        
            })
...
const apiname = async(name)=>{
    try {
    const url = await axios.get(`${URLFOOD}/recipes/complexSearch?query=${name}&addRecipeInformation=true&number=10&apiKey=${YOUR_APY_KEY}`)
...
const infodb = async () => {
    try{
        const datadb =  await Recipe.findAll({ 
            include:{model: Diet,    atributes: ["name"], through:{ attributes: []}
        }
        })
        console.log(datadb)
...
try{
    ...
    // console.log(dataDB)
        if(dataDB && infoByName !== 'error, sin coinicidencias' && infoByName !== 'error'){
            ...
            return res.json(obj)
        }else if(infoByName !== 'error, sin coinicidencias' && infoByName !== 'error'){
            return res.json(infoByName)
        }
     
        else{
       let  objerr = {name: 'only enter numbers less than 100.000.000 or UUID code', 
...
router.get('/:id', async (req, res) => {
   ...
   try {
if (id.length < 8){ 
            const resAxios = await axios.get(`${URLFOOD}/recipes/${id}/information?apiKey=${YOUR_APY_KEY}&addRecipeInformation=true&number=100`);
       ...
       let idfilter ={
        ...
        diets: datefilter.diets?.map(element => element),types: datefilter.dishTypes?.map(element => element), 
        summary:datefilter.summary, 
        steps: (datefilter.analyzedInstructions[0] && datefilter.analyzedInstructions[0].steps?datefilter.analyzedInstructions[0].steps.map(item=>item.step).join((" \n")):'')
       }
